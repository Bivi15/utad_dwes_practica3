...
const createInfo = async (req, res, next) => {
    try{
        ...
        const stats = await calcularStats(
            infoMeteorologicaModel,
            "temperaturaReal",
            { sonda: data.sonda._id }
        );

        notifyClients({
            tipo: "Temperatura_real",
            campo: "temperaturaReal",
            accion: "create",
...
const deleteInfo = async (req, res, next) => {
    try{
        const data  = await infoMeteorologicaModel.findOneAndUpdate({ _id: req.params.id, isDelete: false }, { isDelete: true }, { new: true }).populate("sonda");
        if (!data){
            return errorResponse(res, "Medición no encontrada", BAD_REQUEST);
        }
